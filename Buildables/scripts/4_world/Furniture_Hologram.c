...
{	
	ItemBase FurnitureCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(KMP_Helipad) )
				{
					KMP_Helipad var_KMP_Helipad_Holo = KMP_Helipad.Cast(obj_collided);
					return var_KMP_Helipad_Holo;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(KMP_Helipad_Kit))
			return "KMP_Helipad_Holo";//Testing with Holo, It was - Helipad
    ...
    override void EvaluateCollision( ItemBase action_item = NULL )
    {
        ...
        if ( m_Projection.IsInherited(KMP_Helipad_Kit) || m_Projection.IsInherited(KMP_Helipad_Kit))
		{
