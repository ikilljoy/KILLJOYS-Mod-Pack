...
{	
	ItemBase FurnitureCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(KMP_FootLocker) )
				{
					KMP_FootLocker var_KMP_FootLocker_Holo = KMP_FootLocker.Cast(obj_collided);
					return var_KMP_FootLocker_Holo;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(KMP_FootLocker_Kit))
			return "KMP_FootLocker_Holo";//I CHANGED THIS TO TEST HOLO, IT WAS - KMP_FootLocker
    ...
    override void EvaluateCollision( ItemBase action_item = NULL )
    {
        ...
        if ( m_Projection.IsInherited(KMP_FootLocker_Kit) || m_Projection.IsInherited(KMP_FootLocker_Kit))
		{
