...
{	
	ItemBase FurnitureCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(KMP_KatanaStand) )
				{
					KMP_KatanaStand var_KMP_KatanaStand_Holo = KMP_KatanaStand.Cast(obj_collided);
					return var_KMP_KatanaStand_Holo;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(KMP_KatanaStand_Kit))
			return "KMP_KatanaStand_Holo";//I CHANGED THIS TO TEST HOLO, IT WAS - KMP_KatanaStand
    ...
    override void EvaluateCollision( ItemBase action_item = NULL )
    {
        ...
        if ( m_Projection.IsInherited(KMP_KatanaStand_Kit) || m_Projection.IsInherited(KMP_KatanaStand_Kit))
		{
