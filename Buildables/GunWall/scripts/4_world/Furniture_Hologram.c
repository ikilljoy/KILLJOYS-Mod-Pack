...
{	
	ItemBase FurnitureCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(KMP_GunWall) )
				{
					KMP_GunWall var_GunCabinetHolo = KMP_GunWall.Cast(obj_collided);
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(KMP_GunWall_Kit))
			return "KMP_GunWall_Holo";//I CHANGED THIS TO TEST HOLO, IT WAS - KMP_GunWall
    ...
    override void EvaluateCollision( ItemBase action_item = NULL )
    {
        ...
        if ( m_Projection.IsInherited(KMP_GunWall_Kit) || m_Projection.IsInherited(KMP_GunWall_Kit))
		{
